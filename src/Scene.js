...
Scene.prototype.toObjectBuffer = function() {
	...
	for (let i = 0, l = this.objects.length; i < l; i++) buffer.set(this.objects[i].toBuffer(), i * Sphere.BUFFER_SIZE);
...
Scene.prototype.toMaterialBuffer = function() {
	...
	for (let i = 0, l = this.materials.length; i < l; i++) buffer.set(this.materials[i].toBuffer(), i * Material.BUFFER_SIZE);
